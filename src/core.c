  ...
  do
  {
    ...
    if(access( mod, R_OK ) != -1 )
    {
      ...
      break;
        ...
        WARNPRINT("Can't load module %s at %s, no reading permissions",s, mod);
    }
      ...
      while (!feof(conffile))
	{
      ...
      /* I don't think we need this - seems copy and paste error 
         sprintf(modname,"%s/%s.so",libdir,modules.module[modules.num]->name); */
		  ...
		  modules.module[modules.num]->args=NULL; /*For "stack" type modules we have to set it here, some debug print at the end uses the pointer*/
		  modules.num++;
		}
	    }
		  ...
		  if (modules.module[modules.num]->handle==NULL)
		    {
		      ...
		      if ( dlerr == NULL )
		      {
		          WARNPRINT("Can't load module %s (File not found in PNMPI_LIB_PATH)",modname);
		      }
		      else
		      {
		          WARNPRINT("Can't load module %s (Error %s)",modname,dlerr);
		      }
		    }
		  else
		    {
