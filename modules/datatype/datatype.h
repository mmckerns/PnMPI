...
typedef int (*PNMPIMOD_Datatype_getReference_t)(const void*, int, MPI_Datatype, PNMPIMOD_Datatype_Parameters_t*);
int PNMPIMOD_Datatype_getReference(const void *buf, int count, MPI_Datatype dt, PNMPIMOD_Datatype_Parameters_t *ref);

typedef int (*PNMPIMOD_Datatype_delReference_t)(PNMPIMOD_Datatype_Parameters_t*);
...
{\
  ...
  do {\
	  ...
	  (_ref->stack[_ref->level].b==_ref->stack[_ref->level].dt->list[_ref->stack[_ref->level].i].block))\
	{ \
	  _ref->stack[_ref->level].b=0;\
	  _ref->stack[_ref->level].j++;\
	  if ((_ref->stack[_ref->level].dt->list==NULL) ||\
	      (_ref->stack[_ref->level].j==_ref->stack[_ref->level].dt->list[_ref->stack[_ref->level].i].repeat))\
	    { \
	      _ref->stack[_ref->level].j=0;\
	      _ref->stack[_ref->level].i++;\
	      if ((_ref->stack[_ref->level].i==_ref->stack[_ref->level].dt->count) ||\
		...
		{\
		  _ref->stack[_ref->level].blockbuf=_ref->stack[_ref->level].startbuf+\
		    _ref->stack[_ref->level].dt->list[_ref->stack[_ref->level].i].start;\
		  _ref->stack[_ref->level].buf=_ref->stack[_ref->level].blockbuf;\
		}\
	    }\
	  else\
	    {\
	      _ref->stack[_ref->level].blockbuf += \
		_ref->stack[_ref->level].dt->list[_ref->stack[_ref->level].i].step;\
	      _ref->stack[_ref->level].buf=_ref->stack[_ref->level].blockbuf;\
	    }\
	}\
      else\
	{\
	  if (_ref->stack[_ref->level].b!=0)\
	    _ref->stack[_ref->level].buf+=\
	      _ref->stack[_ref->level].dt->list[_ref->stack[_ref->level].i].dt->size;	\
	}\
\
      if (cont==0)\
	{\
	  while (_ref->stack[_ref->level].dt->homogeneous==0)\
	    {\
	      ...
	      _ref->stack[_ref->level].j=0;\
	      _ref->stack[_ref->level].b=0;\
	      _ref->stack[_ref->level].dt=_ref->stack[_ref->level-1].dt->list[_ref->stack[_ref->level-1].i].dt;\
	      _ref->stack[_ref->level].startbuf=_ref->stack[_ref->level-1].buf;\
	      _ref->stack[_ref->level].blockbuf=_ref->stack[_ref->level].startbuf;\
...
#define PNMPIMOD_Datatype_nextItem(ref,done) PNMPIMOD_Datatype_nextItem_wrap((ref),(done))
...
#define PNMPIMOD_Datatype_getItem(ref,buf,typ,len,size,last) PNMPIMOD_Datatype_getItem_wrap((ref),(buf),(typ),(len),(size),(last))
