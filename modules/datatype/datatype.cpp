...
#define CHECK_DATATYPE(dt) if (datatypetable.find(dt)==datatypetable.end()) { \
...
#define ERASE_DATATYPE(dt,doit) { \
  ...
  if (ptr2->usage==0) { \
    ...
    if (ptr2->list) { free(ptr2->list);} free (ptr2);  \
    if (doit) datatypetable.erase(dt); else datatypetable[dt]=NULL; } }
...
void erase_datatype_rec(PNMPIMOD_Datatype_storage_p ptr,int doit)
{
  ...
  if (ptr->list)
	...
	{
	  ptr->list[i].dt->usage--;
	  ERASE_DATATYPE_FROM_REC(ptr->list[i].dt->store_dt,ptr->list[i].dt,doit);
	}
    }
}
...
#define ALLOC_DATATYPEBASESIZE(trans,mdt,sdt,sz,c,nat) {			\
   trans=(PNMPIMOD_Datatype_storage_p) malloc(sizeof(PNMPIMOD_Datatype_storage_t)); \
   if (trans==NULL) { fprintf(stderr,"ERROR: Can't allocate\n"); exit(1); } \
   trans->count=c; \
   trans->depth=1; \
   trans->size=sz*c; \
   trans->realsize=sz*c; \
...
#define ALLOC_DATATYPENOREG(trans,cnt,mdt,nat) { int _i;			\
   trans=(PNMPIMOD_Datatype_storage_p) malloc(sizeof(PNMPIMOD_Datatype_storage_t)); \
   if (trans==NULL) { fprintf(stderr,"ERROR: Can't allocate\n"); exit(1); }; \
   trans->list=(PNMPIMOD_Datatype_entry_t*) malloc(cnt*sizeof(PNMPIMOD_Datatype_entry_t)); \
   if (trans->list==NULL) { fprintf(stderr,"ERROR: Can't allocate\n"); exit(1); }; \
   for (_i=0; _i<cnt; _i++) { trans->list[_i].repeat=1; trans->list[_i].step=0; \
                              trans->list[_i].block=1; trans->list[_i].start=0;} \
   trans->count=cnt; \
   trans->depth=1; \
   trans->size=0; \
   trans->realsize=0; \
...
#define FIX_SUBTYPES(trans) { \
   ...
   for ( i=0; i<trans->count; i++) { \
     if ((trans->list[i].dt->homogeneous) && (trans->list[i].block>1)) { \
       ...
       ALLOC_DATATYPEBASESIZE(np,trans->list[i].dt->mpi_dt,MPI_DATATYPE_NULL, \
...
#define DEPTH_DATATYPE(trans) { \
   int i,max=1; \
   for ( i=0; i<trans->count; i++) { if (trans->list[i].dt->depth>max) max=trans->list[i].dt->depth; }  \
   trans->depth=max+1; } 
  ...
  do
    {
	  ...
	  (ref->stack[ref->level].b==ref->stack[ref->level].dt->list[ref->stack[ref->level].i].block))
	{
	  ref->stack[ref->level].b=0;
	  ref->stack[ref->level].j++;
	  if ((ref->stack[ref->level].dt->list==NULL) ||
	      (ref->stack[ref->level].j==ref->stack[ref->level].dt->list[ref->stack[ref->level].i].repeat))
	    {
	      ref->stack[ref->level].j=0;
	      ref->stack[ref->level].i++;
	      
	      if ((ref->stack[ref->level].i==ref->stack[ref->level].dt->count) ||
		...
		{
		  ref->stack[ref->level].blockbuf=ref->stack[ref->level].startbuf+
		    ref->stack[ref->level].dt->list[ref->stack[ref->level].i].start;
      ...
      else
	{
	  if (ref->stack[ref->level].b!=0)
	    ref->stack[ref->level].buf+=
	      ref->stack[ref->level].dt->list[ref->stack[ref->level].i].dt->size;	
	}
	  ...
	  while (ref->stack[ref->level].dt->homogeneous==0)
	    {
	      ...
	      ref->stack[ref->level].dt=ref->stack[ref->level-1].dt->list[ref->stack[ref->level-1].i].dt;
	    }
	}
    }
...
void PNMPIMOD_Datatype_getItem(PNMPIMOD_Datatype_Parameters_t *ref,char **buf,MPI_Datatype *typ,int *len, int *size, int *last)
{
#ifdef DATATABLEDEBUG
  map<MPI_Datatype,PNMPIMOD_Datatype_storage_p>::iterator i;
...
int PNMPIMOD_Datatype_getReference( const void *buf, int count, MPI_Datatype dt, PNMPIMOD_Datatype_Parameters_t *ref)
{
  ...
  ref->level=0;
  ref->stack=(PNMPIMOD_Datatype_stack_t*)malloc(sizeof(PNMPIMOD_Datatype_stack_t)*ref->top->depth);
  ref->stack[0].i=0;
  ref->stack[0].j=0;
  ref->stack[0].b=-1;
...
int PNMPIMOD_Datatype_delReference(PNMPIMOD_Datatype_Parameters_t *ref)
{
  ...
  if (ref->top->list) free(ref->top->list);
  if (ref->top) free(ref->top);
  if (ref->stack) free(ref->stack);
  ...
  if (check_level)
      ...
      for (i=datatypetable.begin(); i!=datatypetable.end();  i++)
	{
	  if (i->second->native==0)
	    {
	      if (first) 
		printf("DATATYPE CHECK: Node %i had datatypes that have not been cleaned up\n",dt_rank);
	      first=0;
      ...
      if (first)
	printf("DATATYPE CHECK: all datatypes have been cleaned up on node %i\n",dt_rank);
    }
#endif
...
int MPI_Type_contiguous(int count, MPI_Datatype in_dt, MPI_Datatype *out_dt)
{
  ...
  if (translated_dt->homogeneous)
    {
      ALLOC_DATATYPEBASESIZE(new_dt,translated_dt->mpi_dt,*out_dt,translated_dt->size,count,0)
    }
  else
    {
...
int MPI_Type_vector(int count, int blocklen, int stride, MPI_Datatype in_dt, MPI_Datatype *out_dt)
{
  int ret;
  PNMPIMOD_Datatype_storage_p translated_dt,new_dt;
...
#ifdef DT_DEBUG
  printf("CREATE DT: vector count=%i block=%i stride=%i dt=%i --> %i\n",count,blocklen,stride,in_dt,*out_dt);
#endif

  if ((translated_dt->homogeneous) && ((count==1) || (blocklen==stride)))
    {
      ALLOC_DATATYPEBASESIZE(new_dt,translated_dt->mpi_dt,*out_dt,translated_dt->size,
			     count*blocklen,0);
    }
  else
    {
...
int MPI_Type_hvector(int count, int blocklen, MPI_Aint stride, MPI_Datatype in_dt, MPI_Datatype *out_dt)
{
  int ret;
  PNMPIMOD_Datatype_storage_p translated_dt,new_dt;
  ...
  if ((translated_dt->homogeneous) && ((count==1) || (blocklen*translated_dt->size==stride)))
    {
      ALLOC_DATATYPEBASESIZE(new_dt,translated_dt->mpi_dt,*out_dt,translated_dt->size,
			     count*blocklen,0);
    }
...
int MPI_Type_indexed(int count, int *blocklens, int *indices, MPI_Datatype in_dt, MPI_Datatype *out_dt)
{
...
#ifdef DT_DEBUG
 {
  ...
  hom=1; sum=blocklens[0];
  for (i=1; i<count; i++)
    {
      if (indices[i]!=blocklens[i-1]) hom=0;
      sum+=blocklens[i];
    }
  ...
  if ((translated_dt->homogeneous) && (hom) && (indices[0]==0))
  ...
  else
    {
      ...
      for (i=1; i<count; i++)
	{
	  if ((blocklens[i-1]!=blocklens[i]) || ((stride>=0) && (stride!=indices[i]-indices[i-1]))) hom=0;
	  stride=indices[i]-indices[i-1];
	}
      if (hom)
	{
	  ALLOC_DATATYPE(new_dt,1,*out_dt,0);
	  new_dt->size=translated_dt->size*(blocklens[0]+stride*(count-1));
      ...
      else
	{
	  ...
	  new_dt->size=translated_dt->size*blocklens[count-1]+new_dt->list[count-1].start-new_dt->list[0].start;
	  new_dt->realsize=translated_dt->realsize*sum;
...
int MPI_Type_hindexed(int count, int *blocklens, MPI_Aint *indices, MPI_Datatype in_dt, MPI_Datatype *out_dt)
{
  int ret,i,hom,sum,stride;
  PNMPIMOD_Datatype_storage_p translated_dt,new_dt;
...
#ifdef DT_DEBUG
 {
  ...
  for (i=1; i<count; i++)
    {
      if (indices[i]!=blocklens[i-1]*translated_dt->size) hom=0;
      sum+=blocklens[i];
    }
...
int MPI_Type_struct(int count, int *blocklens, MPI_Aint *indices, MPI_Datatype *in_dts, MPI_Datatype *out_dt)
{
  int ret,i,hom,sum,stride;
  PNMPIMOD_Datatype_storage_p translated_dt,new_dt;
			   ...
			   MPI_Datatype *out_dt)
{
  ...
  ret=PMPI_Type_create_darray(size,rank,ndim,array_of_gsizes,array_of_distribs,
			      array_of_dargs,array_of_psizes, order, in_dt, out_dt);
  return ret;
}
...
int MPI_Type_create_subarray(int ndims, int *array_of_sizes, int *array_of_subsizes,
			     int *array_of_starts, int order, MPI_Datatype in_dt,
			     MPI_Datatype *out_dt)
{
  int ret;
