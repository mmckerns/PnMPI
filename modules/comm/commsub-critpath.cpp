			 ...
			 MPI_Comm comm, void **ptr, int type) 
{
  ...
  if (after_barrier)
    ...
    cp_setedge(critpath_myid,old_timestamp,critpath_myid,local_timestamp,critpath);
		  ...
		  MPI_Comm comm, int err, void **ptr, void **midptr, int type) 
{
  ...
  timing_diff=mytime_convertToSec(mytime_timeDiff(timing_start,mytime_getTimeStamp()));
  ...
  if (after_barrier)
    ...
    {
      if ((ledge)||(verbosity_level))
	cp_setedge(critpath_myid,old_timestamp,critpath_myid,local_timestamp,ledge);
    }
